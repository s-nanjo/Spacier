...
def sklearn_GP(X_train, y_train, X_pool):
    ...
    model = GaussianProcessRegressor(kernel=ConstantKernel()*RBF()+WhiteKernel(), alpha=0, normalize_y=True)
...
def sklearn_GP_st(X_train, y_train, X_pool):
    ...
    model  = make_pipeline(StandardScaler(), GaussianProcessRegressor(kernel=ConstantKernel()*RBF()+WhiteKernel(), alpha=0, normalize_y=True))
    model.fit(X_train, y_train)
    m, s = model.predict(X_pool, return_std=True)
    return m, s
...
def GPy_GP(X_train, y_train, X_pool):
    import GPy
    model = GPy.models.GPRegression(X_train, y_train.values, GPy.kern.RBF(X_train.shape[1]))
    model.optimize()
