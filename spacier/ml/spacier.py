...
class BO():
    def __init__(self, df_X, df_pool_X, df, df_pool, model_name, target, standardization=False):
        q = ""
        for _ in target:
            q+= "(df['" + str(_) + "'] !=-9999)&"
        ...
        if standardization:
            ...
            for _ in range(len(self.target)):
                mu[:, _], sigma[:, _] = eval("model." + self.model_name + \
                                        "(self.df_X[self.index_train,:], self.df_sc[[self.target[_]]], self.df_pool_X)")
        else:          
            for _ in range(len(self.target)):
                mu[:, _], sigma[:, _] = eval("model." + self.model_name + \
                                        "(self.df_X[self.index_train,:], self.df[[self.target[_]]].iloc[self.index_train], self.df_pool_X)")
        self.mu = mu
        self.sigma = sigma
    ...
    def EI(self, query_n):
        ei = np.array([EI_integral(self.mu[_, 0], self.sigma[_, 0], self.df[self.target[0]].max()) for _ in self.index_pool])
    ...
    def EHVI(self, query_n):
        ...
        r = [-5, -5]
        ehvi = np.array([sum_EHVI(PF, r, np.array([self.mu[_, 0], self.mu[_, 1]]), np.array([self.sigma[_, 0], self.sigma[_, 1]]))[0] 
                  for _ in range(len(self.df_pool))] )
        new_index = list(np.argpartition(-ehvi, query_n)[:query_n])
        return new_index
    
def check(df_X):
    if type(df_X) == pd.core.frame.DataFrame:
        return df_X.values
    if type(df_X) == np.ndarray:
        return df_X
    
def PI_integral(mu, sigma, target_range):
    return stats.norm.cdf(target_range[1]-mu, scale=sigma) - stats.norm.cdf(target_range[0]-mu, scale=sigma)  
...
def EI_integral(mu, sigma, y_max):    
    if sigma == 0:
        ...
        t = (mu - y_max) / sigma
        ei = (mu - y_max) * stats.norm.cdf(t) + sigma * stats.norm.pdf(t)
    return ei
...
def sum_EHVI(PF, r, mu, sigma):
    ...
    for i in range(1,len(S)):
        ...
        if 1 - stats.norm.cdf(t)==0:
            ...
            sum_total1+= (y1[i] - y1[i-1]) * (1 - stats.norm.cdf(t)) * psi_calc(y2[i], y2[i], mu[0][1], sigma[0][1])

        sum_total2+= (psi_calc(y1[i-1], y1[i-1], mu[0][0], sigma[0][0]) - psi_calc(y1[i-1], y1[i], mu[0][0], sigma[0][0]))*psi_calc(y2[i], y2[i], mu[0][1], sigma[0][1])      
